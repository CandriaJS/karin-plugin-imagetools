import { getImageInfo } from '@puniyu-plugins/piccy'
...
export const info = karin.command(/^#?(?:柠糖(?:图片操作|imagetools))?(?:查看)?(?:图片信息|imageinfo)$/i, async (e: Message) => {
  try {
    ...
    const image_info = getImageInfo(image_buffer)
    const replyMessage = [
    ...
    if (image_info.isMultiFrame) {
      ...
      replyMessage.push(segment.text(`动图平均帧率: ${image_info.averageDuration}\n`))
    }
